...
export function SettingsEditor({ mode }: { mode: "connections" | "bot" }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      reset(data);
      toast({ tone: "success", title: "Settings saved", description: "The operations configuration is now current." });
    },
...
function BotFields({ form }: { form: Omit<FormParts, "watch"> }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-2">
    ...
    <Section title="Agent runtime" description="Model selection and reasoning profile."><TextField label="Model provider" path="ai.modelProvider" register={register} error={errors.ai?.modelProvider} /><TextField label="Model ID" path="ai.modelId" register={register} placeholder="Leave blank for provider default" /><div><Label htmlFor="thinking">Thinking level</Label><select id="thinking" {...register("ai.thinkingLevel")} className="mt-2 h-10 w-full rounded-md border border-line bg-ink px-3 text-sm text-zinc-100 outline-none focus:border-signal/60">{["off", "minimal", "low", "medium", "high", "xhigh"].map((value) => <option key={value}>{value}</option>)}</select></div></Section>
    <Section title="Conversation memory" description="Control context retention and isolation."><ToggleField label="Enable memory" description="Include recent conversation context in requests." path="memory.enabled" control={control} /><div><Label htmlFor="scope">Memory scope</Label><select id="scope" {...register("memory.scope")} className="mt-2 h-10 w-full rounded-md border border-line bg-ink px-3 text-sm text-zinc-100"><option value="channel_user">Channel/thread + user</option><option value="channel">Shared channel/thread</option></select></div><TextField label="Maximum messages" path="memory.maxMessages" register={register} error={errors.memory?.maxMessages} type="number" /><TextField label="TTL hours" path="memory.ttlHours" register={register} error={errors.memory?.ttlHours} type="number" /><ToggleField label="Include bot replies" description="Retain assistant output alongside user requests." path="memory.includeBotReplies" control={control} /></Section>
    <Section title="Safety & timeouts" description="Bound operations and require human approval."><TextField label="Standard timeout (seconds)" path="timeouts.standardSeconds" register={register} error={errors.timeouts?.standardSeconds} type="number" /><TextField label="Release lookup timeout (seconds)" path="timeouts.releaseLookupSeconds" register={register} error={errors.timeouts?.releaseLookupSeconds} type="number" /><ToggleField label="Require confirmation" description="Confirm repair actions before execution." path="repair.requireConfirmation" control={control} /><ToggleField label="Allow destructive repairs" description="Permit deletion and replacement operations." path="repair.allowDestructive" control={control} /></Section>
  </div>;
}
