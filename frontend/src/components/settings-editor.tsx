...
function BotFields({ form }: { form: Omit<FormParts, "watch"> }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-2">
    ...
    <Section title="Safety and timeouts" description="Set request limits. Repair controls are reserved for a future write executor; current agents are read-only."><TextField label="Standard timeout (seconds)" path="timeouts.standardSeconds" register={register} error={errors.timeouts?.standardSeconds} type="number" /><TextField label="Release lookup timeout (seconds)" path="timeouts.releaseLookupSeconds" register={register} error={errors.timeouts?.releaseLookupSeconds} type="number" /><ToggleField label="Require confirmation" description="Require confirmation if write execution is enabled later." path="repair.requireConfirmation" control={control} /><ToggleField label="Allow destructive repairs" description="Reserved for future file deletion and replacement actions." path="repair.allowDestructive" control={control} /></Section>
  </div>;
}
